...
### «Не делаем» (необязательный забор)
...
Если ни один пункт не проходит тест, секцию «Не делаем» не писать. Пустой заголовок не оставлять.
...
2. **Scope** — эффект для владельца, без пути к файлу и без токена в backticks. «Не делаем» только по тесту забора.
...
- Каждый пункт «Не делаем» проходит три условия забора по тексту, без оценки вероятности. Если ни один не проходит, секции нет.
