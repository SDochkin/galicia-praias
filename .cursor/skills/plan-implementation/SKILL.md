...
**Scope** — для владельца. Полные простые предложения. Что станет иначе на экране или в продукте. Без путей, символов, имён полей и функций. Владелец утверждает план по Scope, не читая Changes.

**Changes** — для агента. Путь, символ, что меняется. Не копировать фразу Scope. Один общий список запрещён: Changes режется на блоки, по одному на каждый пункт Scope, в том же порядке.
...
Число блоков Changes равно числу пунктов Scope. Заголовок блока — номер или короткая метка, не копия фразы Scope. Пункт Changes без своего блока Scope запрещён. Пустой блок запрещён: либо дописать Changes, либо убрать пункт Scope. Общая правка на несколько эффектов живёт в блоке того пункта Scope, без которого она не нужна.
...
2. **Scope** — эффект для владельца, без путей и символов. «Не делаем» только по тесту забора.
...
- В Scope нет путей и символов.
- Changes не пересказывает Scope.
- Число блоков Changes равно числу пунктов Scope.
- В Goal, Scope и Changes нет телеграфа (ярлык вместо фразы, `;` / «потом» между требованиями, нет глагола).
